...
const Prescriptions = () => {
  ...
  const fetchPrescriptions = async () => {
    try {
      ...
      const response = await axios.get(`/prescriptions/patient/${user.id}`);
  ...
  const fetchActivePrescriptions = async () => {
    try {
      ...
      // Use the id property instead of _id
      const response = await axios.get(`/prescriptions/patient/${user.id}/active`);
      if (response.data.success) {
  ...
  const handleDownloadPrescription = async (prescriptionId) => {
    try {
      ...
      // Add authorization headers explicitly
      const token = localStorage.getItem('token');
      const response = await axios.get(`/prescriptions/${prescriptionId}/download`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
