...
exports.forgotPassword = async (req, res) => {
    try {
        const { accountEmail, notificationEmail } = req.body;

        if (!accountEmail || !notificationEmail) {
            return res.status(400).json({
                success: false,
                message: 'Both account email and notification email are required'
            });
        }
        ...
        const message = `You are receiving this email because you (or someone else) has requested the reset of a password for the account ${accountEmail}. Please make a request to: \n\n ${resetUrl}\n\nIf you did not request this, please ignore this email.`;
